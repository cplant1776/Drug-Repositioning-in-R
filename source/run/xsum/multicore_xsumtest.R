...
source("my_connectivity_score2.R")
result <- do.call(c, mclapply(seq_len(ncol(matrix_run)), function(indexrun) {
  indexVector <- matrix_run[,indexrun]
  cat(indexVector[1], '\n')

  drugZscores <- h5read(lincslevel4, index=list(NULL, indexVector), name = "/0/DATA/0/matrix")	#HERE
  H5close()
  ...
  connectivities <- sapply(seq_len(ncol(drugZscores)), function(indexCol) {
    ...
    experimentDrug <- as.matrix(unlist(tapply(drugZscore[IndexDrug], matchGeneDrug, mean))[commonGenes])
    experimentDrug <- experimentDrug[order(experimentDrug),]
